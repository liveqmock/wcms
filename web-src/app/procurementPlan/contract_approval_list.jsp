...
<script type="text/javascript">
	...
	$(function(){
		...
		$('#contract_list').datagrid({
			url: '<%=request.getContextPath()%>/contract/getAuditList?seeStatus=' + seeStatus,
			toolbar:[{iconCls : 'icon-search', text : '查看', 
				handler: function(){
			...
			columns:[[
				...
				{field:'sum',title:'总金额',width:80,align:'right',
					formatter:function(value, rowData, rowIndex){
						return fmoney(value);
					}
				},
		...
		$('#audit-pass').on('click', function(){
			var data = {"id":$('#app_contractId').val()};
	...
	//审批采购订单
	function appContract(data){
		$('#editWin').window({"title":"审批    采购订单编号:"+data.code});
		$('#add_detail_contractId').val(data.id);
		$("#atta_contractId").val(data.id);
		$('#app_contractId').val(data.id);
		...
		$("#app_code").html(data.code);
		$("#app_name").html(data.name);
		$("#app_supplier").html(data.supplier);
		$("#app_supplierLinkman").html(data.supplierLinkman);
		$("#app_supplierPhone").html(data.supplierPhone);
		$("#app_signUser").html(data.signUserName);
		$("#app_signDate").html(data.signDateStr);
		$("#app_sum").html(data.sum);
		$("#app_receiveUser").html(data.receiveUserName);
		$("#app_receiveTime").html(data.receiveTimeStr);
		$("#app_remark").html(data.remark);
	...
	function submitAudit(data, option){
		MaskUtil.mask();
		$.ajax({
			url:'<%=request.getContextPath()%>/contract/submitAudit?id='+data.id+'&option='+option+'&seeStatus='+seeStatus,
			type:'post',
			dataType:'json',
			success:function(data){
				MaskUtil.unmask();
...
<body>
	...
	<div id="editWin" class="easyui-window" collapsible="false" minimizable="false"
    	...
    	<form action="#" method="post" id="editForm">
    		<input type="hidden" name="id" id="app_contractId"/>
   			<table style="width: 100%;">
   				<tr style="line-height: 25px;">
   					<td style="width: 8%; text-align: right;">订单号：</td>
    	  			<td style="width: 15%; text-align: left;" id="app_code">
    	  			</td>
    	  			<td style="width: 8%; text-align: right;">订单名称：</td>
    	  			<td style="width: 15%; text-align: left;" id="app_name">
    	  			</td>
    	  			<td style="width: 8%; text-align: right;">签单人：</td>
    	  			<td style="width: 8%; text-align: left;" id="app_signUser">
    	  			</td>
    	  			<td style="width: 8%; text-align: right;">总金额(元)：</td>
    	  			<td style="width: 8%; text-align: left;" id="app_sum">
    	  			</td>
    	  		</tr>
    	  		<tr style="line-height: 25px;">
    	  			<td style="width: 8%; text-align: right;">签订日期：</td>
    	  			<td style="width: 8%; text-align: left;" id="app_signDate">
    	  			</td>
    	  			<td style="text-align: right;">供应商：</td>
    	  			<td style="text-align: left;" id="app_supplier">
    	  			</td>
    	  			<td style="text-align: right;">联系人：</td>
    	  			<td style="text-align: left;" id="app_supplierLinkman">
					</td>
					<td style="text-align: right;">联系电话：</td>
    	  			<td style="text-align: left;" id="app_supplierPhone">
					</td>
				</tr>
    	  		<tr style="line-height: 25px;">
    	  			<td style="text-align: right;">到货日期：</td>
    	  			<td style="text-align: left;" id="app_receiveTime">
    	  			</td>
    	  			<td style="width: 8%; text-align: right;">收货人：</td>
    	  			<td style="width: 8%; text-align: left;" id="app_receiveUser">
    	  			</td>
    	  			<td style="text-align: right;"></td>
    	  			<td style="text-align: left;">
					</td>
    	  		...
    	  		<tr style="line-height: 25px;">
    	  			<td style="text-align: right;">备注：</td>
    	  			<td style="text-align: left;" colspan="8" id="app_remark">
    	  			</td>
    	  		</tr>
    	  	</table>
    	  	<div style="height: 10px;"></div>
